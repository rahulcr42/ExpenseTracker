...
exports.getAllExpense = async (req, res) => {
  try {
    const expense = await Income.find({ userId: req.user.id }).sort({ date: -1 });
...
exports.deleteExpense = async (req, res) => {
  try {
    ...
    if (!expense || income.userId.toString() !== req.user.id) {
...
exports.downloadExpenseExcel = async (req, res) => {
    ...
    try {
      ...
      // Define file path
      const filePath = path.join('iexpense_details.xlsx');
